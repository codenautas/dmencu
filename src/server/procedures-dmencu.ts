...
export type generarIdEncFun = (area:number,index:number)=>IdEnc

var generarIdEncFun: generarIdEncFun;  
var maxAEncPorArea = 999
...
setGenerarIdEncFun((area:number,numEnc:number)=>area.toString() + numEnc.toString().padStart(maxAEncPorArea.toString().length, '0') as unknown as IdEnc);

setMaxEncPorArea(999);

async function generarNuevoIdEnc(context:ProcedureContext,operativo: IdOperativo, area: number, maxAEncPorArea:number) {
    const siguienteNumeroEncuesta = (await context.client.query(`
        SELECT obtener_siguiente_contador_area($1,$2,$3) AS next_counter;
    `, [operativo, area, maxAEncPorArea]).fetchUniqueValue()).value;
    return generarIdEncFun(area, siguienteNumeroEncuesta);
}
    ...
    if (permite_generar_muestra){
        ...
        const resultUpdate = await context.client.query(`
            UPDATE tem 
                ...
                returning *
        `,[OPERATIVO, encAutogeneradoDm, token, respuestasRaiz, respuestasRaiz]).fetchOneRowIfExists();
        // hago insert porque no existe en tem
        const defaultTarea = await getDefaultTarea(context, OPERATIVO);
        if(resultUpdate.rowCount == 0){
            idEnc = await generarNuevoIdEnc(context, OPERATIVO, area, maxAEncPorArea)
        ...
        await context.client.query(`
            ...
            returning *
        `,[OPERATIVO, idEnc, defaultTarea.tarea, recepcionista, asignado]).fetchUniqueRow();
        await context.client.query(`
            update tareas_tem
                set cargado_dm = $4, estado = $5, operacion = $6
                where operativo= $1 and enc = $2 and tarea = $3
            returning *
        `,[OPERATIVO, idEnc, defaultTarea.tarea, token, ESTADO_POSTERIOR_CARGA, cambia_modo_dm?null:OPERACION_PREPARAR_CARGA]
        ).fetchUniqueRow();
    }
    return idEnc
}
...
const generarEncuestaTem = async (context: ProcedureContext, operativo: IdOperativo, idEnc: IdEnc, area: number, dominio: number, tarea_actual:IdTarea) => {
    await context.client.query(`
        INSERT into tem (operativo, enc, area, dominio, habilitada, tarea_actual) values ($1, $2, $3, $4, $5, $6)`,
    [operativo, idEnc, area, dominio, true, tarea_actual]).execute();
}
...
select o.id_casillero as id_formulario, o.unidad_analisis, 'BF_'||o.casillero boton, bf.casillero, bf.padre, fp.casillero, fp.unidad_analisis
  ...
  where o.tipoc = 'F'
    ...
    },
    {
        ...
        coreFunction:async function(context: ProcedureContext, parameters: CoreFunctionParameters<{operativo: string, persistentes: AnyObject}>){
            var be=context.be;
    ...
    },
    {
        ...
        coreFunction:async function(context: ProcedureContext, parameters: CoreFunctionParameters<{operativo: string, caso: string}>){
            var {be, client} =context;
            let param_proc={
                operativo: parameters.operativo,
    ...
    },
    {
        ...
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, area: number, dominio: number, cant_encuestas: number, tarea_actual: string}>){
            const be =  context.be;
            var permiteGenerarMuestra = (await context.client.query(`
                select permite_generar_muestra 
                    from operativos 
            ...
            if(permiteGenerarMuestra){
                ...
                for(let i = 1; i < params.cant_encuestas - totalActual ; i++){
                    const enc = await generarNuevoIdEnc(context, params.operativo, params.area, maxAEncPorArea)
                    await generarEncuestaTem(context, params.operativo, enc, params.area, params.dominio, params.tarea_actual);
                }
    ...
    {     
        ...
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, enc: string}>){
    ...
    },
    {
        ...
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, tarea: string, enc: string, condicion: string, accion: EstadoAccion}>){
            var be =  context.be;
    ...
    },
    {
        ...
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, tarea: string, enc: string}>){
            await context.client.query(`
                UPDATE tareas_tem
                    set operacion = $4
    ...
    },
    {
        ...
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, enc: string, tarea: string}>){
            await context.client.query(`
                UPDATE tem
    ...
    },
    {
        ...
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, enc: string, tarea_actual: string, tarea_nueva: string}>){
    ...
    {     
        ...
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, encuestador: string}>){
            var result = await context.client.query(`
                select *
                    from tareas_tem
    ...
    },
    {
        ...
        roles:['coor_proc','coor_campo','admin'],
        coreFunction:async function(context:ProcedureContext, params:CoreFunctionParameters<{operativo: string, token: string}>){
            var be = context.be;
    ...
    },
    {
        ...
        roles:['admin'],
        coreFunction:async function(context:ProcedureContext, parameters: CoreFunctionParameters<{operativo: string, enc: string}>){
            (await context.client.query(
                `update tem
